...
pars.save_dir = fullfile(pars.save_dir,'_Appeal_Results');
    ...
    if strcmpi(pars.spiking_inputs{1},'M1_spikes')
            ...
            if strcmpi(pars.monkey,'chewie') || strcmpi(pars.monkey,'mihili')
                ...
                [master_td, pars_td] = loadTDfiles(  files, ...
                            ...
                            {@dimReduce,struct('signals',pars.spiking_inputs,'algorithm',pars.dimReduce_algorithm,'num_dims',max(pars.mani_dims))}, ...
...
% remove trials without the right indices
master_td = removeBadTrials(master_td,struct('remove_nan_idx',true,'nan_idx_names',{{pars.idx_start{1},pars.idx_end{1}}}));
disp('REMOVING NAN IDX! Just so you know.')
...
% Equalize
master_td           = equalNbrTrialsSessions(master_td_all_trials);
