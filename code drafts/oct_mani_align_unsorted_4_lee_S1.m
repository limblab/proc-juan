...
monkey          = 'Han'; % 'mihili'; 'chewie'

unsorted_yn     = true;

spiking_inputs  = {'S1_spikes'}; % 'PMd_spikes'
...
idx_start       = {'idx_goCueTime',0}; % {'idx_movement_on',0}; % {'idx_go_cue',0}
% When we stop looking at each trial --if empty, the duration of the shortest trial 
% -- also the code first trims then downsamples, so this number is in the
% original bins (10 ms)
idx_end         = {'idx_goCueTime',35}; % {''}; % {'idx_go_cue',10}
...
master_td       = loadTDfiles(  '/Users/juangallego/Documents/NeuroPlast/Data/Han/all_TDs_Han.mat', ...
                            {@getTDidx,'result','R'});
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SOME QUICK FIXES TO TRY OUT THE MAIN ANALYSIS

% get rid of bump trials
idx_bump        = ~isnan([master_td.idx_bumpTime]);
master_td(idx_bump) = [];

% get rid of trials with a NaN target
idx_nan_target = isnan([master_td.target_direction]);
master_td(idx_nan_target) = [];


% In only a subset of trials there appear to be an instructed delay, get
% rid of them !
delay_tgtOn_goCue = [master_td.idx_goCueTime] - [master_td.idx_tgtOnTime];
idx_delay_tgtOn_goCue = delay_tgtOn_goCue > 0;

master_td(idx_delay_tgtOn_goCue) = [];
...
% The first few sessions don't have targets 2, 4, 6 and 8 --For the moment
% get rid of them
idx_targets_to_discard = [2 4 6 8];
for t = 1:length(idx_targets_to_discard)
    
    discard = getTDidx(master_td,{'target_direction',targets(idx_targets_to_discard(t))});
    master_td(discard) = [];
end
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Only keep a subset of sessions, to avoid having too few sessions --What
% the hell did I mean here?
...
for s = 1:length(sessions)
    ...
    [~,idx_no_uea] = setdiff(master_td(this_s(1)).S1_unit_guide(:,1),1:96);
    ...
    for t = 1:length(this_s)
        master_td(this_s(t)).S1_unit_guide(idx_del,:)=[];
        master_td(this_s(t)).S1_spikes(:,idx_del)=[];
    end
    
    % ---------------------------------------------------------------------
    % store the nbr of good units per session
    n_units(s) = size(master_td(this_s(1)).S1_unit_guide,1);
    % store channel numbers good units
    units_session{s} = master_td(this_s(1)).S1_unit_guide(:,1);
end
...
if unsorted_yn
    ...
    for s = 1:length(sessions)
        this_s = getTDidx( master_td,{'date',sessions{s}} );
        [t_master_td_new, t_pca_info] = getPCA( master_td(this_s), struct(...
                                    'signals',spiking_inputs,...
                                    'sqrt_transform',true,...
                                    'do_smoothing',true,...
                                    'kernelSD',0.1 ));
        master_td_new = [master_td_new, t_master_td_new];
        pca_info{s} = t_pca_info;
    end
    ...
    figure,hold on
    for s = 1:length(sessions)
        plot(cumsum(pca_info{s}.eigen)/sum(pca_info{s}.eigen)*100)
    end
    set(gca,'TickDir','out','FontSize',14), ylim([0 100])
    xlabel('Number of neural modes'),ylabel('Neural variance explained (%)')
end
...
%     
%     td1 = dupeAndShift(td1,{'S1_spikes',hist_bins});
%     td2 = dupeAndShift(td2,{'S1_spikes',hist_bins});
%     
%     % define model parameters --move up!!!
%     mod_params.in_signals = 'S1_spikes_shift';
...
%     acrossR2(c,:) = R2diff;
%     
%     n_units(c) = size(td1(1).S1_unit_guide,1);
% end
% 
% 
% 
