%
% Do CCA aligning by movement (EMG) onset or the unfolding of the neural
% population dynamcs
%


clearvars -except datasets; close all; clc;
...
% Signal for trial-alignment
align_signal = 'summed_emgs'; % 'summed_emgs'; 'summed_modes'; 'neural_modes'; 'emgs'

% To exclude trials based on weird movement onsets
min_nbr_bins_beginning_mov_onset = 10;
min_nbr_bins_mov_onset_end = 30;
max_diff_mov_onset = 30;

% where to start a trial
trial_begins = min_nbr_bins_beginning_mov_onset - 1;
...
% Neural modes to add and get "population information" in neural space
modes_to_add = 1:6;

% pause for each plot (in s)
sec_pause = 0;
...
for d = 1:length(reach_ds)
    ...
    for i = 1:size(ssc1,3)
        ...
        switch align_signal
            ...
            case 'summed_emgs'
                ...
                if nbr_emgs_use1 > 0 && nbr_emgs_use2 > 0
                    ...
                    if ~isempty(xmin1) && ~isempty(xmin2) 
                        ...
                        % ========================================================
                        % TO DELETE
                        % Plot most of the stuff, for debugging
                        clf
            ...
            case 'summed_modes'
                ...
                tssc1 = ssc1(:,modes_to_add,:);
                tot_ssc1 = sum(tssc1,2).^2;
                
                tssc2 = ssc2(:,modes_to_add,:);
                tot_ssc2 = sum(tssc2,2).^2;
                
                
                figure
                for i = 1:size(tot_ssc1,2)
                    hold on
                    plot(tot_ssc1(:,i)/max(tot_ssc1(:,i)),'c');
                    plot(tot_ssc2(:,i)/max(tot_ssc2(:,i)),'color',[.7 .7 .7]);
                    pause;
                    clf
                end
    ...
    stdata_oldway = equalize_single_trial_dur( stdata_oldway, ...
        'time_win', proj_params.time_win(reach_ds(d),:) );
    ...
    all_r = [all_r; r];
    all_r_oldway = [all_r_oldway; r_oldway];
...
for d = 1:length(reach_ds)
    ...
    plot(all_r_oldway(d,:),'-.','linewidth',2,'color',colors(d,:));
end
set(gca,'Tickdir','out'),set(gca,'FontSize',14), box off,
xlabel('Neural mode'), ylabel('Cross-task canonical correlation')
legend('align to mov. onset','align to go signal'), legend boxoff
...
figure,hold on
for d = 1:length(reach_ds)
    plot(all_r(d,:)./all_r_oldway(d,:),'linewidth',2,'color',colors(d,:));
    %plot(all_r_oldway(d,:),'-.','linewidth',2,'color',colors(d,:));
end
