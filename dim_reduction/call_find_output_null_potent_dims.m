...
function [onp_dim_raw, onp_dim_summary, single_trial_data ] = call_find_output_null_potent_dims( ...
                        data_struct, neural_chs, chosen_emgs, labels, varargin )
...
% 1.a. bin and smooth the FRs (if not passed as inputs)
...
% 1.b. crop the binned_data between the desired words

% in the iso sessions, all of the tasks are WF
if sum(strncmp(labels,'iso',3)) > 0
    % create tasks array for cropping
    for i = 1:nbr_bdfs, tasks{i} = 'wf'; end
    cropped_binned_data     = call_crop_binned_data( binned_data, w_i, w_f, tasks );
else
    % change the label of any MG task to 'mg'
    tasks                   = labels;
    for i = 1:length(binned_data)
        if strncmp(tasks{i},'mg',2), tasks{i} = 'mg'; end
    end
    cropped_binned_data     = call_crop_binned_data( binned_data, w_i, w_f, tasks );
end
...
for i = 1:numel(lags_to_include)
    if sum(ismember(lags_best_fit,lags_to_include(i))) == 0
        ...
        warning(['neuron to EMG delay' num2str(lags_to_include(i)) ...
            ' s did not provide the best fit but I am including it']);
    end
