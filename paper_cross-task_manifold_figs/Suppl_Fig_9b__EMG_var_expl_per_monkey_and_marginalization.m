%
% Plot EMG variance per marginalization, pooled over all sessions for
...
for m = 1:length(s_p_monkey)
    ...
    for s = 1:length(s_p_monkey{m})
        ...
        expl_var{m}(s,:) = dPCA_emg_results{s_p_monkey{m}(s)}.expl_var.totalMarginalizedVar / ...
                        dPCA_emg_results{s_p_monkey{m}(s)}.expl_var.totalVar * 100;
    end
end
...
% create matrices with all datapoints
...
% plot, including individual datapoints
figure, hold on
for m = 1:length(marg_labels)
    for k = length(s_p_monkey):-1:1
        if k == 1 % for monkey 1 empty bars
            ...
            plot(3*m-1+.25, expl_var{k}(:,marg_swap(m)), 'o', 'color', pt_cols, 'linestyle', 'none')
            ...
            plot(3*m-2+.25, expl_var{k}(:,marg_swap(m)), 'o', 'color', pt_cols, 'linestyle', 'none')
        end
    end
end
set(gca,'TickDir','out','FontSize',14), box off
ylabel('EMG var. expl. (%)'), ylim([0 60])
